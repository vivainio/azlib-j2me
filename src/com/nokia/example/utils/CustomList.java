...
import com.nokia.example.utils.gesture.*;
...
public class CustomList
    ...
    public final void setTheme(Theme theme) {
        ...
        for (int i = 0; i < elements.size(); i++) {
            ((Element) elements.elementAt(i)).resetCaches();
        }
    ...
    public boolean isSelected(int elementNum)
        ...
        try {
            return ((Element) elements.elementAt(elementNum)).isSelected();
        }
        catch (ArrayIndexOutOfBoundsException e) {
            throw new IndexOutOfBoundsException(e.getMessage());
        }
    ...
    public int getSelectedIndex() {
        for (int i = 0; i < elements.size(); i++) {
            if (((Element) elements.elementAt(i)).isSelected()) {
                return i;
            }
        }
        return -1;
    ...
    public int getSelectedFlags(boolean[] selectedArray_return) {
        ...
        for (int i = 0; i < selectedArray_return.length; i++) {
            selectedArray_return[i] =
                ((Element) elements.elementAt(i)).isSelected();
    ...
    public void setSelectedIndex(int elementNum, boolean selected)
        throws IndexOutOfBoundsException {
        try {
            ((Element) elements.elementAt(elementNum)).setSelected(selected);
            updateListCommand();
        }
        catch (ArrayIndexOutOfBoundsException e) {
            throw new IndexOutOfBoundsException(e.getMessage());
    ...
    public void setSelectedFlags(boolean[] selectedArray)
        ...
        for (int i = 0; i < selectedArray.length; i++) {
            ((Element) elements.elementAt(i)).setSelected(selectedArray[i]);
        }
        updateListCommand();
    }
    ...
    private void setFocusedElementIndex(int focusedElementIndex) {
    ...
    public void scrollToElement(int elementNum) {
        if (elementNum >= 0 && elementNum < elements.size()) {
            ...
            for (int i = 0; i < elements.size(); i++) {
                if (i < elementNum) {
                    topY += ((Element) elements.elementAt(i)).getHeight();
                }
                else if (i == elementNum) {
                    bottomY = topY
                        + ((Element) elements.elementAt(i)).getHeight();
                }
            }
    ...
    private void handleSelectEvent(SafeGestureEvent event) {
        ...
        if (elementIndex > -1) {
            ...
            if (listType == List.MULTIPLE) {
                Element element = (Element) elements.elementAt(elementIndex);
                element.setSelected(!element.isSelected());
            }
            else {
                for (int i = 0; i < elements.size(); i++) {
                    ((Element) elements.elementAt(i)).setSelected(
                        elementIndex == i);
                }
            }
            updateListCommand();
    ...
    protected void handleSelectEvent() {
        if (focusedElementIndex > -1 && focusedElementIndex < elements.size()) {
            if (listType == List.MULTIPLE) {
                Element element = (Element) elements.elementAt(
                    focusedElementIndex);
                element.setSelected(!element.isSelected());
            }
            else {
    ...
    protected int elementIndexAt(int x, int y) {
        ...
        for (int i = 0; i < elements.size(); i++) {
            ...
            heightNext += ((Element) elements.elementAt(i)).getHeight();
    ...
    protected int calculateHeight() {
        ...
        for (int i = 0, size = elements.size(); i < size; i++) {
            h += ((Element) elements.elementAt(i)).getHeight();
        }
        return h;
    }
    ...
    protected void drawElements(Graphics g, int yOffset,
        ...
        for (int i = 0, size = elements.size(); i < size; i++) {
            ...
            drawElement(g, i, heightSoFar + yOffset,
